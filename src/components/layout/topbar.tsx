...
const headerMap: Record<string, { title: string; subtitle: string }> = {
  ...
  "fines-monitoring": { title: "Мониторинг штрафов", subtitle: "Статистика штрафов, исков и подключение сервисов проверки." },
...
export function Topbar({ onMenuClick }: { onMenuClick?: () => void }) {
  ...
  return (
    <header className={`premium-header relative mb-3 flex flex-wrap items-center justify-between gap-2 rounded-2xl p-2 backdrop-blur-xl ring-1 ${accent.ring} sm:mb-4 sm:gap-3 sm:p-3`}>
      ...
      <div className="relative z-10 flex w-full flex-wrap items-center gap-2 lg:w-auto">
        {showOverviewButton ? (
        ...
        <button aria-label="Открыть уведомления" className="rounded-xl border border-slate-200 bg-white/90 p-2.5 text-muted shadow-sm transition hover:-translate-y-0.5">
          <Bell size={16} />
        </button>
        <button onClick={refreshData} className="rounded-xl border border-slate-200 bg-white/90 px-3 py-2 text-xs font-semibold text-slate-700 shadow-sm transition hover:-translate-y-0.5 md:text-sm">
